...
class Conv2D(object):
    ...
    def backward(self, x, dv_y):
        ...
        dv_x_padded = np.zeros(x_padded.shape, dtype=np.float32)

        for f in range(self.W.shape[0]):
            for i in range(dv_y.shape[1]):
                for j in range(dv_y.shape[2]):
                    dreceptive_field = dv_y[f, i, j] * self.W[f]
                    pi = i * s[0]
                    pj = j * s[1]
                    dv_x_padded[:, pi:pi + self.kernel_size[0], pj:pj + self.kernel_size[1]] += dreceptive_field
                    dv_W[f] += dv_y[f, i, j] * x_padded[:, pi:pi + self.W.shape[2], pj:pj + self.W.shape[3]]
                    dv_b[f] += dv_y[f, i, j]

        dv_x = dv_x_padded[:, p[0]:p[0] + x.shape[1], p[1]:p[1] + x.shape[2]]
        # don't change the order of return values
        return dv_x, dv_W, dv_b
...
class MaxPool2D:
    ...
    def backward(self, x, dv_y):
        ...
        # TODO: write your implementation below
        dv_x_padded = np.zeros(x_padded.shape)
        dv_x = np.zeros(x.shape)

        for t in range(dv_y.shape[0]):
            for i in range(dv_y.shape[1]):
                for j in range(dv_y.shape[2]):
                    window = x_padded[t, i * s[0]: i * s[0] + self.kernel_size[0],
                             j * s[1]: j * s[1] + self.kernel_size[1]]
                    max_of_window = np.max(window)
                    dv_x_padded[t, i * s[0]: i * s[0] + self.kernel_size[0],
                    j * s[1]: j * s[1] + self.kernel_size[1]] += (window == max_of_window) * dv_y[t, i, j]
        dv_x = dv_x_padded[:, p[0]:p[0] + x.shape[1], p[1]:p[1] + x.shape[2]]
        return dv_x
