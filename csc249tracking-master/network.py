...
def complex_conj_multiply(x, z):
    out_real = x[..., 0] * z[..., 0] + x[..., 1] * z[..., 1]
    out_imaginary = x[..., 1] * z[..., 0] - x[..., 0] * z[..., 1]
    return torch.stack((out_real, out_imaginary), -1)
...
class DCFNet(nn.Module):
    ...
    def forward(self, z):
        ...
        responsef = torch.sum(complex_conj_multiply(zf, self.wf), dim = 1, keepdim = True)
        response = torch.irfft(responsef, signal_ndim = 2)
    ...
    def update(self, x, lr=1.0):
        ...
        xf_s = torch.sum(torch.sum(xf ** 2, dim = 4, keepdim = True), dim = 1, keepdim = True)
        wf = complex_conj_multiply(xf, self.config.yf) / ( xf_s + self.config.lambda0)
        if lr > 0.99:
            self.wf = wf
            self.xf = xf
        else:
