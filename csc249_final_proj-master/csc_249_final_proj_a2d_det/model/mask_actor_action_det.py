    ...
    def forward(self, x):
        ...
        for i in range(self.num_levels):
            ...
            if self.pool_type == 'max_pool':
                tensor =  tensor = F.max_pool2d(x, kernel_size=kernel_size, stride=stride, padding=pooling).view(num, -1)
            else:
...
class featureNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(b):
            for j in range(s):
                with torch.no_grad():
                    out = self.resnet(x[i,j,:])
                    out = self.ssplayer(out)
                    outs.append(out)
        outs = outs.view(b,s,-1)
        return outs
...
class FasterRCNNA2D(nn.Module):
    ...
    def forward(self, data, flow, im_info, roidb=None, **rpn_kwargs):
        ...
        with torch.set_grad_enabled(self.training):
            ...
            """
            action_blob_cov = []
            segment_action_blob_conv_pyramid = self.Conv_Body(segment_flow)
            for segment_action_blob_conv_level_feat in segment_action_blob_conv_pyramid:
                indices = [(2 * k + 1) * int(cfg.A2D.SEGMENT_LENGTH / 2) for k in range(b)]
                frame_feature_level = torch.index_select(segment_action_blob_conv_level_feat, dim=0, index=
                torch.tensor(indices).to(segment_action_blob_conv_level_feat.device))
                action_blob_conv.append(frame_feature_level)
                """
